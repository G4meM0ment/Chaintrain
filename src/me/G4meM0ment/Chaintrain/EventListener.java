...
public class EventListener implements Listener {
	...
	Messages messages;
	public EventListener(Chaintrain chaintrain, Messages messages)
	{
	...
	public void onPlayerJoin(PlayerJoinEvent event)
	{
		...
		// in it's own thread because it takes some time and would stop the rest of the world to load
			if(chaintrain.getConfig().getBoolean("checkVersion") && (player.isOp() || player.hasPermission("dar.admin"))) {
				new Thread() {
					public void run() {
						try {
							...
							if (!chaintrain.getDescription().getVersion().equalsIgnoreCase(line)) {
								messages.newVersion(player, line);
							}
							reader.close();
		...
		new Thread() {
			public void run() {
				...
				while (counter < time && isChaining.get(name)) {
					...
					messages.chaining(player, counter+1);
					messages.getChained(target, counter+1);
		    		if(chaintrain.getConfig().getBoolean("useSpout"))
		    		{
				...
				if(counter >= time) {					
					if(chaintrain.isChained(target))
					{
			    		...
			    		messages.unchained(target, player);
					}
					else if(!chaintrain.isChained(player))
					{
			    		...
			    		messages.chained(target, player);
					}
				}
			}
		}.start();
